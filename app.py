        ...
        if len(all_data) > 1:
            ...
            if 'දිනය' in df.columns:
                ...
                if not this_month_df.empty:
                    ...
                    # --- LIST VIEW (වර්ණ ගැන්වූ ලැයිස්තුව) ---
st.write("---")
...
# පෙන්විය යුතු Columns ටික තෝරා ගැනීම
...
# --- අලුතින් එකතු කළ Styling කොටස ---
def apply_color(row):
    # 'වර්ගය' අනුව පාට තීරණය කිරීම
    if row['වර්ගය'] == 'වියදම්':
        color = 'color: red;'
    elif row['වර්ගය'] == 'ආදායම්':
        color = 'color: blue;'
    else:
        color = 'color: black;'
    
    # පේළියේ ඇති සියලුම තීරුවලට එම පාට ලබාදීම
    return [color] * len(row)

# DataFrame එකට style එක apply කිරීම
styled_df = this_month_df[final_cols].style.apply(apply_color, axis=1)

# Styled table එක පෙන්වීම
