...
# --- 📊 MONTHLY SUMMARY (ROBUST VERSION) ---
st.markdown("---")
...
if sheet:
    try:
        # වෙනස්කම 1: get_all_records වෙනුවට get_all_values භාවිතා කිරීම (වඩා ආරක්ෂිතයි)
        all_data = sheet.get_all_values()
        
        if len(all_data) > 1: # Header එකට අමතරව දත්ත තියෙනවා නම්
            # Header සහ Data වෙන් කර ගැනීම
            headers = all_data[0]
            rows = all_data[1:]
            
            # DataFrame එක අතින් සැකසීම
            ...
            # 2. මුදල් පිරිසිදු කිරීම (ඉතා වැදගත් කොටස)
            if 'මුදල' in df.columns:
                # මුලින්ම හිස්තැන් (Empty strings) 0 බවට පත් කිරීම
                df['මුදල'] = df['මුදල'].replace(r'^\s*$', '0', regex=True)
                # Rs., කොමා, සහ අකුරු ඉවත් කිරීම (ඉලක්කම් සහ තිත පමණක් ඉතිරි කරයි)
                df['මුදල'] = df['මුදල'].astype(str).str.replace(r'[^\d.]', '', regex=True)
                # අන්තිමට ඉලක්කම් (Numeric) බවට හැරවීම
                df['මුදල'] = pd.to_numeric(df['මුදල'], errors='coerce').fillna(0)
            
            # 3. දිනය පිරිසිදු කිරීම
